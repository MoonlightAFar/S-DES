    ...
    public static String transform(String Input_Text, String Key,int state) {
        ...
        if(state==0) {//加密
            ...
            for(int i=0;i<Input_Text.length();i++) {
                ...
                //对二进制子明文进行加密，得到子密文
                String Sub_Chipher_Bits = sdes.SDES(Sub_Plain_Bits,Key,state)[0];
            ...
            for(int i=0;i<Input_Text.length();i++) {
                ...
                //对二进制子密文进行解密，得到子明文
                String Sub_Plain_Bits = sdes.SDES(Sub_Cipher_Bits,Key,state)[0];;
